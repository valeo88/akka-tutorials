...
class TestProbesSpec extends TestKit(ActorSystem("TestProbeSpec"))
  with ImplicitSender
  ...
  "A master actor" should {
    "register a slave" in {
       ...
       * We can use TestProbe like stub of real actor.
       */
    ...
    "aggregate data correctly" in {
      ...
      // `workloadString` - match exact the same object = workloadString
      slave.receiveWhile() {
        case SlaveWork(`workloadString`, `testActor`) => slave.reply(WorkCompleted(3, testActor))
      }

      // check first send 3, then 6
      expectMsg(Report(3))
      expectMsg(Report(6))
    }
  }
...
object TestProbesSpec {
  ...
  // we don't need Slave implementation to test interaction between requester -> master -> slave
}
